  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="p-4 bg-white dark:bg-[#1E293B] border border-[#E5E7EB] dark:border-[#334155] rounded-[12px] shadow-2xs flex items-center justify-between">
          <div>
            <p className="text-xs text-gray-500 dark:text-[#94A3B8] font-medium">Total Rent Collected</p>
            <p className="text-lg font-bold text-[#16A34A] dark:text-emerald-400 mt-0.5">
              ₹{totalCollected.toLocaleString('en-IN')}
            </p>
          </div>
          <div className="p-2.5 bg-green-50 dark:bg-emerald-950/80 text-[#16A34A] dark:text-emerald-400 rounded-xl">
        ...
        <div className="p-4 bg-white dark:bg-[#1E293B] border border-[#E5E7EB] dark:border-[#334155] rounded-[12px] shadow-2xs flex items-center justify-between">
          <div>
            <p className="text-xs text-gray-500 dark:text-[#94A3B8] font-medium">Pending Dues</p>
            <p className="text-lg font-bold text-[#F59E0B] dark:text-amber-400 mt-0.5">
              ₹{totalPending.toLocaleString('en-IN')}
            </p>
          </div>
          <div className="p-2.5 bg-amber-50 dark:bg-amber-950/80 text-[#F59E0B] dark:text-amber-400 rounded-xl">
        ...
        <div className="p-4 bg-white dark:bg-[#1E293B] border border-[#E5E7EB] dark:border-[#334155] rounded-[12px] shadow-2xs flex items-center justify-between">
          <div>
            <p className="text-xs text-gray-500 dark:text-[#94A3B8] font-medium">Overdue Invoices</p>
            <p className="text-lg font-bold text-[#DC2626] dark:text-red-400 mt-0.5">
              {payments.filter((p) => p.status === 'Overdue').length} Invoices
            </p>
          </div>
          <div className="p-2.5 bg-red-50 dark:bg-red-950/80 text-[#DC2626] dark:text-red-400 rounded-xl">
      ...
      {/* Filters Bar */}
      <div className="p-3 bg-white dark:bg-[#1E293B] border border-[#E5E7EB] dark:border-[#334155] rounded-[12px] flex items-center justify-between gap-3 shadow-2xs">
        <div className="flex items-center gap-2 text-xs">
          <Filter className="w-3.5 h-3.5 text-gray-400 dark:text-[#64748B]" />
          <span className="text-gray-500 dark:text-[#94A3B8] font-medium">Status Filter:</span>
        ...
        <span className="text-xs text-gray-500 dark:text-[#94A3B8]">
          Showing <strong className="text-[#111827] dark:text-[#F8FAFC]">{filteredPayments.length}</strong> payment record(s)
        </span>
      </div>
        ...
        <CardContent className="p-0">
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs text-[#111827] dark:text-[#F8FAFC]">
              <thead className="bg-slate-50 dark:bg-[#1E293B] border-b border-[#E5E7EB] dark:border-[#334155] text-gray-500 dark:text-[#CBD5E1] font-semibold uppercase tracking-wider text-[11px]">
                <tr>
              ...
              <tbody className="divide-y divide-[#E5E7EB] dark:divide-[#334155]">
                {filteredPayments.length === 0 ? (
                  <tr>
                    <td colSpan={7} className="px-4 py-8 text-center text-gray-500 dark:text-[#94A3B8]">
                  ...
                  filteredPayments.map((p) => {
                    ...
                    return (
                      <tr key={p.id} className="hover:bg-slate-50/80 dark:hover:bg-[#273549]/50 transition-colors">
                        {/* 1. Invoice Number (Clickable to open Payment Details) */}
                        <td className="px-4 py-3.5">
                          <button
                            type="button"
                            onClick={() => handleOpenDetails(p)}
                            className="font-mono font-bold text-[#2563EB] dark:text-blue-400 hover:underline flex items-center gap-1.5 focus:outline-hidden cursor-pointer"
                        ...
                        {/* 2. Warehouse */}
                        <td className="px-4 py-3.5 font-medium text-gray-800 dark:text-[#CBD5E1]">{p.warehouseName}</td>

                        {/* 3. Tenant */}
                        <td className="px-4 py-3.5 font-medium text-gray-900 dark:text-[#F8FAFC]">{p.tenantName}</td>

                        {/* 4. Due Date */}
                        <td className="px-4 py-3.5 text-gray-600 dark:text-[#94A3B8] font-mono">{p.dueDate}</td>

                        {/* 5. Amount */}
                        <td className="px-4 py-3.5 font-bold text-[#111827] dark:text-[#F8FAFC]">
