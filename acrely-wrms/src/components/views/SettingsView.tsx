...
export const SettingsView: React.FC<SettingsViewProps> = ({
  ...
  return (
    <div className="space-y-6 max-w-5xl">
      ...
      {/* Role Switcher Card */}
      <Card className="border-[#2563EB]/30 bg-blue-50/20 dark:bg-blue-950/20">
        <CardHeader>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <UserCheck className="w-5 h-5 text-[#2563EB] dark:text-blue-400" />
        ...
        <CardContent className="space-y-3">
          ...
          <div className="flex items-center gap-3 pt-1">
            <button
              ...
              className={`flex-1 p-3 rounded-[12px] border text-left transition-all cursor-pointer ${
                userRole === 'Owner' || userRole === 'Property Owner'
                  ? 'border-[#2563EB] bg-white dark:bg-[#1E293B] shadow-sm ring-2 ring-[#2563EB]/20'
                  : 'border-[#E5E7EB] dark:border-[#334155] bg-slate-50 dark:bg-[#1E293B]/50 hover:bg-white dark:hover:bg-[#1E293B]'
              }`}
            >
              <div className="flex items-center justify-between mb-1">
                <span className="font-bold text-xs text-[#111827] dark:text-[#F8FAFC]">Property Owner</span>
                {(userRole === 'Owner' || userRole === 'Property Owner') && <Badge variant="success">Active</Badge>}
              </div>
              <p className="text-[11px] text-gray-500 dark:text-[#94A3B8]">Full administrative control (Can Add, Edit, and Delete all entities).</p>
            </button>
            ...
            <button
              ...
              className={`flex-1 p-3 rounded-[12px] border text-left transition-all cursor-pointer ${
                userRole === 'Editor'
                  ? 'border-[#F59E0B] bg-white dark:bg-[#1E293B] shadow-sm ring-2 ring-[#F59E0B]/20'
                  : 'border-[#E5E7EB] dark:border-[#334155] bg-slate-50 dark:bg-[#1E293B]/50 hover:bg-white dark:hover:bg-[#1E293B]'
              }`}
            >
              <div className="flex items-center justify-between mb-1">
                <span className="font-bold text-xs text-[#111827] dark:text-[#F8FAFC]">Staff Associate / Editor</span>
                {userRole === 'Editor' && <Badge variant="warning">Active</Badge>}
              </div>
              <p className="text-[11px] text-gray-500 dark:text-[#94A3B8]">Restricted access (Can View & Edit, but cannot permanently delete data).</p>
            </button>
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <div className="relative">
              ...
              <input
                ...
                className="pl-8 pr-3 py-1.5 border border-[#E5E7EB] dark:border-[#334155] bg-white dark:bg-[#1E293B] text-xs text-[#111827] dark:text-[#F8FAFC] rounded-[8px] w-48 focus:outline-none focus:border-[#2563EB]"
              />
            </div>
        ...
        <CardContent className="p-4 space-y-3">
          {filteredStaff.length > 0 ? (
            ...
            <div className="p-6 bg-slate-50 dark:bg-[#1E293B]/50 border border-dashed border-[#E5E7EB] dark:border-[#334155] rounded-[12px] text-center text-xs text-gray-500 dark:text-[#94A3B8]">
      ...
      <Card>
        ...
        <CardContent>
          {documents.length > 0 ? (
              ...
              {documents.map((doc) => (
                <div key={doc.id} className="py-3 flex items-center justify-between gap-3">
                  <div className="flex items-center gap-3">
                    <div className="p-2 bg-blue-50 dark:bg-blue-950/60 text-[#2563EB] dark:text-blue-400 rounded-[8px]">
                      <FileText className="w-4 h-4" />
                    </div>
                    <div>
                      <span className="text-xs font-bold text-[#111827] dark:text-[#F8FAFC]">{doc.name}</span>
            ...
            <div className="p-4 bg-slate-50 dark:bg-[#1E293B]/50 border border-dashed border-[#E5E7EB] dark:border-[#334155] rounded-[10px] text-center text-xs text-gray-500 dark:text-[#94A3B8]">
      ...
      <div className="pt-2">
        <h2 className="text-sm font-bold text-[#111827] dark:text-[#F8FAFC] mb-3 uppercase tracking-wider text-gray-500 dark:text-[#94A3B8]">
