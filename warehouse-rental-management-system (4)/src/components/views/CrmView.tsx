...
/**
 * CrmView Component
 * Renders the CRM Tenant Pipeline with Kanban & List views, Lead details page `/crm/[lead-id]`,
 * stage progression controls (◀ Previous Stage / ▶ Next Stage), and move confirmation dialogs.
 */
export const CrmView: React.FC<CrmViewProps> = ({
  ...
  // View mode toggle: 'pipeline' (Kanban) or 'list'
  const [viewFormat, setViewFormat] = useState<'pipeline' | 'list'>('pipeline');

  // Currently selected Lead ID for viewing details page (/crm/[lead-id])
  const [selectedLeadId, setSelectedLeadId] = useState<string | null>(null);

  // Stage transition confirmation modal state
  ...
  // Filter leads based on global search query
  ...
  // Find the lead object if a lead details page is active
  const selectedLead = leads.find((l) => l.id === selectedLeadId);

  // If viewing a specific Lead details page (/crm/[lead-id]), render LeadDetailView
  if (selectedLead) {
    return (
      <LeadDetailView
        lead={selectedLead}
  ...
  // Helper function to return stage badge color styles
  ...
  // Trigger stage move confirmation modal
  ...
  // Confirm stage movement and invoke parent callback
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-2 border-b border-[#E5E7EB]">
        ...
        <div className="flex items-center gap-2">
          {/* Format Toggle */}
          <div className="flex items-center bg-white border border-[#E5E7EB] rounded-[10px] p-1 text-xs font-medium">
            <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4 overflow-x-auto pb-4">
          {STAGES.map((stg) => {
            ...
            return (
              <div
                key={stg}
                className="bg-slate-50/70 border border-[#E5E7EB] rounded-[12px] p-3 space-y-3 min-w-[230px]"
              >
                {/* Column Header & Counter */}
                <div className="flex items-center justify-between px-1">
                  <span className="text-xs font-bold text-[#111827]">{stg}</span>
                  <span className="text-[11px] font-semibold text-gray-600 bg-white border border-[#E5E7EB] px-2 py-0.5 rounded-full shadow-2xs">
                    {stageLeads.length}
                  </span>
                </div>
                ...
                <div className="space-y-3">
                  {stageLeads.length === 0 ? (
                    <div className="p-4 bg-white border border-dashed border-[#E5E7EB] rounded-[10px] text-center">
                      <p className="text-[11px] text-gray-400">No active leads in this stage.</p>
                    </div>
                  ) : (
                    stageLeads.map((ld) => {
                        ...
                        >
                          <CardContent className="p-3.5 flex flex-col h-full justify-between">
                            ...
                            <div className="space-y-2.5">
                              {/* Card Header with truncated company & lead name */}
                              <div className="flex items-start justify-between gap-1">
                                <div className="min-w-0 flex-1">
                                  <h4
                                    className="font-bold text-xs text-[#111827] hover:text-[#2563EB] cursor-pointer truncate"
                                    title={ld.name}
                              ...
                              {/* Requirement Summary */}
                              <div className="text-[11px] text-gray-600 space-y-1 bg-slate-50/80 p-2.5 rounded-[8px] border border-[#E5E7EB]">
                                <p className="flex items-center justify-between">
                                  <span className="text-gray-400">Req Area:</span>
                                  <strong className="text-[#111827] truncate ml-1">
                                ...
                                <p className="flex items-center justify-between">
                                  ...
                                  <strong className="text-[#2563EB] truncate ml-1">
                                ...
                                {ld.brokerName && (
                                  <p className="text-[10px] text-gray-500 flex items-center gap-1 mt-1 pt-1 border-t border-[#E5E7EB]/60 truncate">
                            ...
                            <div className="space-y-2 pt-2.5 border-t border-[#E5E7EB] mt-auto">
                              <ViewButton
                                className="w-full"
                                onClick={() => setSelectedLeadId(ld.id)}
                              >
      ...
      {moveModal.isOpen && moveModal.lead && moveModal.targetStage && (
        ...
        >
          <div className="space-y-4">
            <div className="p-4 bg-blue-50/80 border border-blue-200/80 rounded-[12px] space-y-2">
              <p className="text-sm font-semibold text-[#111827]">
                Move &quot;{moveModal.lead.name}&quot; from{' '}
                <span className="font-bold text-[#2563EB]">{moveModal.lead.stage}</span> to{' '}
                <span className="font-bold text-[#2563EB]">{moveModal.targetStage}</span>?
              </p>
