...
export const PaymentHistory: React.FC<PaymentHistoryProps> = ({ history = [] }) => {
  if (history.length === 0) {
    return (
      <div className="p-4 bg-slate-50 border border-[#E5E7EB] rounded-[10px] text-xs text-gray-500 text-center">
  ...
  return (
    <div className="space-y-2.5">
      {history.map((event) => (
        <div
          key={event.id}
          className="p-3 bg-white border border-[#E5E7EB] rounded-[10px] flex items-start gap-3 hover:border-gray-300 transition-colors"
        >
          <div
