...
export default function App() {
  ...
  if (activeTab === 'login') {
    return <LoginView onSignIn={() => setActiveTab('dashboard')} />;
  }

  return (
    <div className="min-h-screen bg-[#F8FAFC] text-[#111827] flex flex-col font-sans antialiased relative">
      {/* Toast Notification Banner */}
      {toast && (
        <div className="fixed top-4 right-4 z-50 flex items-center gap-3 px-4 py-3 bg-white border border-[#E5E7EB] rounded-[12px] shadow-lg animate-in fade-in slide-in-from-top-2">
          {toast.type === 'success' ? (
            <CheckCircle2 className="w-5 h-5 text-[#16A34A] shrink-0" />
          ) : (
            <AlertCircle className="w-5 h-5 text-[#DC2626] shrink-0" />
          )}
          <span className="text-xs font-semibold text-[#111827]">{toast.message}</span>
          <button
            onClick={() => setToast(null)}
            className="text-gray-400 hover:text-[#111827] p-1 rounded-md"
          >
