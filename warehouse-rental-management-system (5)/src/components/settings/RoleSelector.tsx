...
const ROLE_DESCRIPTIONS: Record<StaffRole, { title: string; description: string; icon: React.ReactNode }> = {
  ...
  Broker: {
    ...
    icon: <Briefcase className="w-4 h-4 text-[#2563EB] dark:text-blue-400" />
  },
  Custom: {
    title: 'Custom Role',
    description: 'Tailored permissions matrix configured granularly per module and operation type.',
    icon: <Wrench className="w-4 h-4 text-[#2563EB] dark:text-blue-400" />
  }
};
...
export const RoleSelector: React.FC<RoleSelectorProps> = ({
  ...
  return (
    <div className="space-y-3">
      <label className="block text-xs font-semibold text-[#111827] dark:text-[#CBD5E1]">
          ...
          return (
            <button
              ...
              className={`p-3 rounded-[10px] border text-left transition-all cursor-pointer ${
                isSelected
                  ? 'border-[#2563EB] bg-blue-50/40 dark:bg-blue-950/40 ring-2 ring-[#2563EB]/20 shadow-2xs'
                  : 'border-[#E5E7EB] dark:border-[#334155] bg-white dark:bg-[#1E293B] hover:bg-slate-50 dark:hover:bg-[#273549]'
              } ${disabled ? 'opacity-60 cursor-not-allowed' : ''}`}
            >
              <div className="flex items-center gap-2 mb-1">
                {info.icon}
                <span className="font-bold text-xs text-[#111827] dark:text-[#F8FAFC]">{info.title}</span>
              </div>
              <p className="text-[11px] text-gray-500 dark:text-[#94A3B8] leading-relaxed">{info.description}</p>
            </button>
          );
        })}
      </div>
