      ...
      <div className="space-y-6">
        ...
        <div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <div className="p-3 bg-white dark:bg-[#1E293B] border border-[#E5E7EB] dark:border-[#334155] rounded-[10px]">
              <div className="flex items-center gap-1.5 text-xs text-gray-500 dark:text-[#94A3B8] mb-1">
                <Maximize className="w-3.5 h-3.5 text-blue-600 dark:text-blue-400" />
            ...
            <div className="p-3 bg-white dark:bg-[#1E293B] border border-[#E5E7EB] dark:border-[#334155] rounded-[10px]">
              ...
              <p className="text-sm font-semibold text-[#111827] dark:text-[#F8FAFC]">{warehouse.powerCapacity}</p>
            </div>
            ...
            <div className="p-3 bg-white dark:bg-[#1E293B] border border-[#E5E7EB] dark:border-[#334155] rounded-[10px]">
              ...
              <p className="text-sm font-semibold text-[#111827] dark:text-[#F8FAFC]">{warehouse.ceilingHeightFt} Ft</p>
            </div>
          ...
          {documents.filter(d => !d.associatedEntity || d.associatedEntity.includes(warehouse.name)).length > 0 ? (
            <div className="space-y-2">
              {documents.filter(d => !d.associatedEntity || d.associatedEntity.includes(warehouse.name)).map((doc) => (
                <div key={doc.id} className="p-3 bg-white dark:bg-[#1E293B] border border-[#E5E7EB] dark:border-[#334155] rounded-[10px] flex items-center justify-between">
                  <div className="flex items-center gap-2.5">
                    <FileText className="w-4 h-4 text-[#2563EB] dark:text-blue-400" />
                    <div>
                      <p className="text-xs font-semibold text-[#111827] dark:text-[#F8FAFC]">{doc.name}</p>
                      <p className="text-[10px] text-gray-500 dark:text-[#94A3B8]">{doc.category} • {doc.fileSize} • Uploaded {doc.uploadedDate}</p>
                    </div>
                  </div>
                  {onDeleteDocument && (
